...
if (1==1):
    ...
    ##### Specific for HNDT ##########
    path_to_rkf = "//Users/janani/Desktop/Raman_trials/ADF_new_version_trials/H2O/H2O_30b_vcd/H2O.results/H2O_vcd_dftb.rkf"
    mol = plams.Molecule("/Users/janani/Desktop/Raman_trials/ADF_new_version_trials/H2O/H2O_30b_vcd/H2O_3ob_vcd.xyz")
    print(mol)
    ...
    print("vector V", V)
    print ("length of Vector V", len(V[1]))

    
    # Creates array with 3*N x nModes
    numHess = np.empty([rkfFiles[0].nAtoms*3,len(V[1])])
    # Creates array with 9 x nModes
    polTens = np.empty([9,len(V[1])])
    print("poltensor", polTens)
    ...
    for mode in range(len(V[1])):
        ...
        polTens[:,mode] = ((pol_plus - pol_less) / (2*scaling))*bohr_to_ang #bohr to ang conv included
        numHess[:,mode] = ((grad_plus - grad_less) / (2*scaling))*bohr_to_ang #bohr to ang conv included
